...
1. The two illustrated snippets, run for real against `evals/bench/data/retest-2026-08-31.csv`
   and `evals/bench/data/soak-2026-08-27.csv`, produce the exact figures the recipe page states.
...
2. The range check in `_as_plausible_volts` catches the retest export's millivolts-under-a-volts-
   header column before any snippet runs, on its own and end to end through `run`.
...
from evals.bench import RETEST_CSV, SOAK_CSV  # noqa: E402
