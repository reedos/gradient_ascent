...
from examples.evaluator_optimizer.__main__ import SCRIPTED  # noqa: E402

CORPUS_DIR = ROOT / "evals" / "corpus"
QUESTION = "How often should the DW-300's filter be cleaned?"

# The same four replies examples/evaluator_optimizer/__main__.py scripts for `--model
# stub:scripted`: a first draft that fails the checker, then a revision that passes it.
SEQUENCE = [
...
class ScriptedCommandTests(unittest.TestCase):
    ...
    def test_the_scripted_sequence_shows_a_failing_draft_then_a_passing_revision(self) -> None:
        # The page's own claim (a first draft fails the checker, a revision passes) only holds if
        # this exact sequence produces exactly one revision and a citation the checker accepted,
        # not a draft that happens to pass immediately.
