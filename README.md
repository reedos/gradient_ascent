...
| `scripts/` | `validate.py` (content rules), `eval_run.py` (the eval runner), `record_trace.py` (records one run), `import_eci.py` (refreshes the capability snapshot; one of two scripts that uses the network), `check_sources.py` (fetches every URL the site cites and reports the ones that moved, drifted or are gone; the other script that uses the network), `review_queue.py` (every page by its `reviewed` date, oldest first, flagged past 90 days), `preview_server.py` (serves the built site under its base path). |
| `tests/` | The Python suite. Stdlib `unittest`, no network, no fixtures outside the repository. |
| `content/` | `taxonomy.json` (the levels, tracks, recipes, teardowns and typed edges between pages), `landscape.json` (the registry of named models, products and tools), `glossary.json`, `timeline.json`, `shapes.json` (the kinds of job, by the shape of the work), `frontier.json` (what is unsolved at each level), `changes.json` (the change log behind `/changes/` and its feed), `worksheet.json`, and `capability.json` (a dated snapshot of Epoch AI's Epoch Capabilities Index, CC BY 4.0, drawn on the timeline page). |
