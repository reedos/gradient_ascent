...
No model is called at any level of this example, so `--model` is accepted only because every
example here takes it, and it is ignored.
"""
from __future__ import annotations

from examples.common.cli import parse_args
from examples.common.trace import Tracer
from examples.household_paperwork.run import LEVEL, run


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv or __import__("sys").argv[1:], description="Level 0: household paperwork, no model.")
