...
searches, it does not call a model. Embedding always uses the deterministic stub unless a real
`Embedder` is wired in by hand; there is no `--embedder` flag here because the site's other
examples do not have one either, and this one has no answer-generation step to pair it with.
"""
