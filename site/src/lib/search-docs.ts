...
export async function buildSearchDocs(): Promise<SearchDoc[]> {
  ...
  const views: { id: string; title: string; summary: string; path: string }[] = [
    {id:'chatbot-to-agent',title:'From a chatbot to an always-on agent',summary:'The evolution of conversational AI through context, workflows, tools, agent loops, optional teams, and persistent operation. Architectural changes explained with diagrams.',path:'/chatbot-to-agent/'},
