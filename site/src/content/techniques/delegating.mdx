...
- <Link href="/levels/0/">Conventional software</Link>: nothing is delegated; the code does the whole task by
  a fixed rule you wrote.
- <Link href="/levels/1/">Direct prompting</Link>: you delegate the drafting and keep every decision,
  because nothing happens until you act on the reply.
- <Link href="/levels/2/">Added context</Link>: you also delegate the search, so "how checkable" now
...
- <Link href="/levels/4/">Tool use</Link>: the reversibility row stops being hypothetical, because
  now something outside the conversation actually changes.
- <Link href="/levels/5/">Agent loops</Link>: you delegate a sequence you will not see, so score the
