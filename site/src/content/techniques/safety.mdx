...
- <Link href="/levels/0/">Conventional software</Link>: there is no model to redirect, so the risks here are
  ...
  own concerns.
- <Link href="/levels/1/">Direct prompting</Link>: the user's own message is the only input in play, the
...
- <Link href="/levels/2/">Added context</Link>: retrieved text can carry instructions of its own, which
...
- <Link href="/levels/4/">Tool use</Link>: a tool call can act on the world, not just produce a
  ...
  earns its place.
- <Link href="/levels/5/">Agent loops</Link>: the model chooses its own next step across many turns
