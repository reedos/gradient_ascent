...
- <Link href="/levels/0/">Conventional software</Link>: there is no model call to grade, so this is ordinary
  ...
  rather than sampled.
- <Link href="/levels/1/">Direct prompting</Link>: grading one prompt's answer is the simplest case;
...
- <Link href="/levels/2/">Added context</Link>: what got retrieved changes the answer, so grading has
...
- <Link href="/levels/4/">Tool use</Link>: grading has to check whether the right tool was called
  ...
  reads fine.
- <Link href="/levels/5/">Agent loops</Link>: the model decides when to stop, so an eval has to weigh
