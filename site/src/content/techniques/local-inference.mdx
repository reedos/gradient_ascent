...
- <Link href="/levels/0/">Conventional software</Link>: nothing to run locally: this topic's own concerns
  start at level 1.
- <Link href="/levels/1/">Direct prompting</Link>: the smallest case (one short prompt, one short
  answer), the least memory this page's own estimator will ever report for a given model.
- <Link href="/levels/2/">Added context</Link>: a retrieved passage set or a large pasted document
...
- <Link href="/levels/4/">Tool use</Link>: tool definitions and their results are extra tokens in
...
- <Link href="/levels/5/">Agent loops</Link>: a long-running loop's transcript keeps growing across
