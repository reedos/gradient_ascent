...
<CostStrip
  ...
  comparedTo={{
    ...
    note: 'Similar model work can have similar per-run cost, but schedules may add unnecessary calls. Event filters, ordinary rules, and deduplication can skip model calls when there is no useful work. Measure the actual trigger policy and workload.',
  }}
  level={7}
/>
