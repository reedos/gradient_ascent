...
- <Link href="/levels/0/">Conventional software</Link>: there is no output to review, only code to test.
- <Link href="/levels/1/">Direct prompting</Link>: one reply, reviewed against what you already know or
  can look up in the time you were willing to spend.
- <Link href="/levels/2/">Added context</Link>: the sources are on screen, so the cheap checks become
...
- <Link href="/levels/4/">Tool use</Link>: a proposed action can be reviewed before it runs, which
  is a cheaper and more useful check than reading about it afterwards.
- <Link href="/levels/5/">Agent loops</Link>: nobody reads the whole run, so review becomes a sample
