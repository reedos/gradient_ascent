...
<TryIt items={[
  { lane: 'use', body: 'Open a chat app\'s project or custom-instructions feature. Put a document there instead of pasting it into a message, ask about it, then ask something unrelated in the same project. Does it still see the document?' },
  { lane: 'build', body: 'Run python -m examples.context_engineering --model stub:scripted from the repo root, then lower TOKEN_BUDGET in examples/context_engineering/run.py below the static block\'s 6,934 tokens. The history budget floors at zero, nothing is cut, and the prompt goes out over budget: it governs history, not the block the cache depends on.' },
  { lane: 'either', body: 'Cause a failure mode above on purpose, with the documents in evals/corpus/.' },
]} />
