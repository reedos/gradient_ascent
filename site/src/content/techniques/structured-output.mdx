...
through a schema in `response_format`<Cite n={2}/>. Anthropic supports schema-constrained JSON
responses through `output_config.format`, and separately supports strict tool inputs through
`strict: true`. These can be used independently or together<Cite n={3}/>. A direct structured reply
