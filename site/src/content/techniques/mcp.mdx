...
This page is sourced, not measured: every claim about MCP is checked against the protocol's own
specification and the makers' pages, and no server has been run and scored here. It is
illustrated.
...
only resources is read-only and safe to add freely; one offering tools can take actions on your
