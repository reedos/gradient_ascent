...
counts coming from paraphrases written down in advance rather than from a model. The same command with `--model stub` keeps nothing and rejects all 32 at verification: the
echoing stub's placeholder answer matches no real grading pattern. That is the check working on an
input that was never trying to be right.
