...
- <Link href="/levels/0/">Conventional software</Link>: nothing to calibrate: a rule passes its tests or it
  does not, and it behaves the same way tomorrow.
- <Link href="/levels/1/">Direct prompting</Link>: every reply is independent, so the record is simply a
  tally per task type with nothing else to attribute a change to.
- <Link href="/levels/2/">Added context</Link>: the record has to separate "found the right source and
...
- <Link href="/levels/4/">Tool use</Link>: the record now needs to cover the action taken, not only
  the sentence produced: a wrong call can be reported in perfectly correct prose.
- <Link href="/levels/5/">Agent loops</Link>: the unit becomes a whole run of variable length, so the
