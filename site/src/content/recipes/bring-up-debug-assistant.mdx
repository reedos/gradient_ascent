...
That last question is the job. It is not whether this unit is in spec, a comparison the test
executive already made; it is what the log and one more reading say about what to check next. The
...
DMM and the load terminals would simply agree, both near zero, and the log already had the answer.
Run it the other way, read the log and then go straight to failure analysis, and it scraps a good
board and a good fixture cycle to fix nothing. The order that is right depends on the shape of the
first answer, which is exactly what a level 5 loop is for and a checklist is not.
...
through that same checked sequence. The agent's own tools never touch `SafetyEnvelope`,
`GuardedSupply`, `GuardedLoad`, or `Approval` at all, because nothing it can call reaches them:
...
<CostStrip
  stats={[
    ...
    { label: 'Tokens out, cumulative', value: '~110' },
  ]}
  comparedTo={{
...
what one would say. No board on this bench gets an agent call by default: this recipe runs on a
failure that already needs a person's attention, not on every unit that passes.
...
<FailureModes modes={[
  {
    ...
    notice: "MEAS:VOLT:DC? 0.1 reads as a read-only header, and is_read_only agrees, but the DMM uses that argument to set its DC range before taking the reading. Send it once and every later query on that meter, argument or not, comes back through the same wrong range, which on this board reads as an overload with an empty error queue.",
    test: "Send a read-only header with an argument it does not need and confirm the range, or any other instrument state, is unchanged afterward. examples/bench_bring_up_debug_assistant/run.py refuses any argument outside the one header (MEAS:VPP? on the scope) that actually needs one, rather than trusting is_read_only alone; see this recipe's page-request against examples/common/bench.py.",
  },
  {
