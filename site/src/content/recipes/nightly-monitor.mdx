...
<Link href="/recipes/literature-watch/">A weekly watch over a growing result set</Link> works that
...
- Swap the mechanism for a source that grows instead of changing: where a query returns a result
  ...
  night's copy, which is the same shape and cheaper arithmetic, and is what <Link
  href="/recipes/weekly-status-report/">the weekly status report</Link> and <Link
  href="/recipes/literature-watch/">the literature watch</Link> both do.
## Design choices
