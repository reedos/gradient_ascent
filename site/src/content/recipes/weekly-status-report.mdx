...
The office manager at a twelve-person firm sends the same email every Friday afternoon: what
moved this week, what is behind, and what somebody has to deal with before Monday. Putting it
together takes an hour and a half, and almost none of that is writing. It is opening the project
tracker and counting what closed, opening the shared client inbox and working out which threads
have been sitting on the firm's side too long, opening the spreadsheet where somebody types the
hours, and adding them up against what each project was budgeted. Three systems, three different
shapes, three different degrees of trustworthiness, and one email at the end of it.

This is the job people mean when they say they want AI to do their reporting, and most of it
turns out not to need one. The counting, the comparing and the schedule are arithmetic. What a
model is genuinely good for here is the last twenty minutes: turning a table nobody wants to read
into four paragraphs they will.

Not in scope: writing anything back to any of the three systems, which is
<Link href="/recipes/project-tracker-upkeep/">a different recipe and a harder one</Link>; deciding
whether a project is in trouble; and sending the email. The report comes out with its own
problems named on it, and a person reads it before anyone else does.
...
A one-off report is assembled by somebody who already knows what happened. A standing one runs
whether or not anything interesting happened, on a day nobody chose for a reason, over sources
nobody looked at first. Four things follow, and all four are in the code rather than in the prose.

**The window comes from the schedule, not from judgment.** The run covers 09/11/2026 to
09/18/2026 because that is where the last report stopped. A task closed on the day of the previous
report belongs to that one. Without a fixed window, tasks get counted twice or not at all,
depending on when somebody happened to run it.

**A quiet week still produces a report.** A standing job that goes silent when nothing happened is
indistinguishable from one that broke, and a team learns within a month to stop noticing either.

**A stale source is not a quiet week.** The time spreadsheet says it is current only to
09/15/2026, three days behind the report. Reporting zero hours for the week would be a lie code
could have caught, so the figures carry the date the hours actually run through and the report is
required to say so. That is a mild version of a failure that does real damage in
<Link href="/recipes/project-tracker-upkeep/">the tracker recipe</Link>, where the same silence
ages a document instead of spoiling one email.

**Nobody is watching the inputs, so the code has to speak up.** One project name in the inbox
matched nothing on the firm's list. In a report assembled by hand, somebody would have seen it.
Here it is a figure like any other and it appears in the email.
